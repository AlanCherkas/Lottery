...
namespace Game.Services.Concrete
{
	public class LotteryService : ILotteryService
	{
		...
		public void StartLottery()
		{
			...
			MessageDisplayService.DisplayWelcomeMessage(players.FirstOrDefault(p => p.Number == PlayersConstants.DefaultUserPlayerNumber), TicketsConstants.DefaultTicketPrice);
			
			TicketService.PurchaseTickets(players);

			List<WinnerDto> winners = PrizeCalculationService.CalculatePrize(players);
			double houseProfit = PrizeCalculationService.CalculateHouseProfit(players, winners);
